...
class TextAreaLanguage:
    """A container for a language which has been registered with the PersistentPlaceholderTextArea."""
...
class PersistentPlaceholderTextArea(ScrollView):
    DEFAULT_CSS = """\
PersistentPlaceholderTextArea {
    width: 1fr;
    height: 1fr;
...
"""
    ...
    """
    `PersistentPlaceholderTextArea` offers some component classes which can be used to style aspects of the widget.
    ...
    it first using  [`PersistentPlaceholderTextArea.register_language`][textual.widgets._text_area.PersistentPlaceholderTextArea.register_language].
    """

    theme: Reactive[str] = reactive("css", always_update=True, init=False)
    """The name of the theme to use.

    Themes must be registered using  [`PersistentPlaceholderTextArea.register_theme`][textual.widgets._text_area.PersistentPlaceholderTextArea.register_theme] before they can be used.
    ...
    The text selected in the document is available via the `PersistentPlaceholderTextArea.selected_text` property.
    """
    ...
    Changing this value will immediately re-render the `PersistentPlaceholderTextArea`."""
    ...
    If `False`, the cursor will be hidden when `read_only==True`, and the PersistentPlaceholderTextArea will
    ...
    """Hide suggestion when the PersistentPlaceholderTextArea does not have focus."""
    ...
    class Changed(Message):
        """Posted when the content inside the PersistentPlaceholderTextArea changes.

        Handle this message using the `on` decorator - `@on(PersistentPlaceholderTextArea.Changed)`
        or a method named `on_text_area_changed`.
        """

        text_area: PersistentPlaceholderTextArea
        """The `text_area` that sent this message."""

        @property
        def control(self) -> PersistentPlaceholderTextArea:
            """The `PersistentPlaceholderTextArea` that sent this message."""
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new `PersistentPlaceholderTextArea`.

        Args:
            text: The initial text to load into the PersistentPlaceholderTextArea.
            ...
            name: The name of the `PersistentPlaceholderTextArea` widget.
        ...
        registered by end-users using `PersistentPlaceholderTextArea.register_language`. If a user attempts
        to set `PersistentPlaceholderTextArea.language` to a language that is not registered here, we'll
        ...
        self.set_reactive(PersistentPlaceholderTextArea.soft_wrap, soft_wrap)
        self.set_reactive(PersistentPlaceholderTextArea.read_only, read_only)
        self.set_reactive(PersistentPlaceholderTextArea.show_cursor, show_cursor)
        self.set_reactive(PersistentPlaceholderTextArea.show_line_numbers, show_line_numbers)
        self.set_reactive(PersistentPlaceholderTextArea.line_number_start, line_number_start)
        self.set_reactive(PersistentPlaceholderTextArea.highlight_cursor_line, highlight_cursor_line)
        self.set_reactive(PersistentPlaceholderTextArea.placeholder, placeholder)
    ...
    def code_editor(
        ...
        *,
        ...
        """Construct a new `PersistentPlaceholderTextArea` with sensible defaults for editing code.

        This instantiates a `PersistentPlaceholderTextArea` with line numbers enabled, soft wrapping
    ...
    def update_suggestion(self) -> None:
        """A hook to update the [`suggestion`][textual.widgets.PersistentPlaceholderTextArea.suggestion] attribute."""
    ...
    def _set_theme(self, theme: str) -> None:
        ...
        try:
            ...
            if theme_object is None:
                raise ThemeDoesNotExist(
                    ...
                    f"then switch to that theme by setting the `PersistentPlaceholderTextArea.theme` attribute."
                ) from None
        ...
        if theme_object:
            ...
            if base_style:
                ...
                # the PersistentPlaceholderTextArea background/color should fallback to its CSS colors.
                #
    ...
    def available_themes(self) -> set[str]:
        """A list of the names of the themes available to the `PersistentPlaceholderTextArea`.
    ...
    def register_theme(self, theme: TextAreaTheme) -> None:
        """Register a theme for use by the `PersistentPlaceholderTextArea`.

        After registering a theme, you can set themes by assigning the theme
        name to the `PersistentPlaceholderTextArea.theme` reactive attribute. For example
    ...
    def available_languages(self) -> set[str]:
        """A set of the names of languages available to the `PersistentPlaceholderTextArea`.

        The values in this set can be assigned to the `language` reactive attribute
        of `PersistentPlaceholderTextArea`.
    ...
    def register_language(
        ...
        Calling this method does not change the language of the `PersistentPlaceholderTextArea`.
        ...
        Registering a language only registers it to this instance of `PersistentPlaceholderTextArea`.
    ...
    def _set_document(self, text: str, language: str | None) -> None:
        ...
        if TREE_SITTER and language:
            ...
            # User has supplied a language i.e. `PersistentPlaceholderTextArea(language="python")`, but they
            ...
            log.warning(
                ...
                "and it's highlight query using PersistentPlaceholderTextArea.register_language().\n\n"
    ...
    def load_text(self, text: str) -> None:
        """Load text into the PersistentPlaceholderTextArea.

        This will replace the text currently in the PersistentPlaceholderTextArea and clear the edit history.

        Args:
            text: The text to load into the PersistentPlaceholderTextArea.
        """
        self.history.clear()
    ...
    def is_syntax_aware(self) -> bool:
        """True if the PersistentPlaceholderTextArea is currently syntax aware - i.e. it's parsing document content."""
    ...
    def _refresh_size(self) -> None:
        """Update the virtual size of the PersistentPlaceholderTextArea."""
        if self.soft_wrap:
    ...
    def get_line(self, line_index: int) -> Text:
        ...
        styling to PersistentPlaceholderTextArea content.
    ...
    def render_line(self, y: int) -> Strip:
        ...
        if not self.text and self.placeholder:
            ...
            logger.write("\n")
            logger.write(str(placeholder_lines))
    ...
    def _render_line(self, y: int) -> Strip:
        ...
        line_len = len(line)
        # line += self.placeholder[line_len-1:]
    ...
    def text(self, value: str) -> None:
        """Replace the text currently in the PersistentPlaceholderTextArea. This is an alias of `load_text`.

        Setting this value will clear the edit history.

        Args:
            value: The text to load into the PersistentPlaceholderTextArea.
        """
    ...
    async def _on_mouse_down(self, event: events.MouseDown) -> None:
        ...
        # PersistentPlaceholderTextArea widget while selecting, the widget still scrolls.
        self.capture_mouse()
        self._pause_blink(visible=False)
        self.history.checkpoint()
    ...
    def scroll_cursor_visible(
        ...
        """Scroll the `PersistentPlaceholderTextArea` such that the cursor is visible on screen.
    ...
    def select_all(self) -> None:
        """Select all of the text in the `PersistentPlaceholderTextArea`."""
    ...
    def cursor_location(self) -> Location:
        ...
        This is a utility for accessing the `end` of `PersistentPlaceholderTextArea.selection`.
        """
